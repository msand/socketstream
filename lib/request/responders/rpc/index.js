...
var fs, log;
var RpcStream = require('./rpcstream');
fs = require('fs');
  ...
  return {
    ...
    interfaces: function(middleware) {
      ...
      return {
        ...
        websocket: function(msg, meta, send, stream) {
          ...
          log.debug('↪'.cyan, msgLogName, req.method);
          ...
          handleError = function(e) {
            ...
            log.error('↩'.red, msgLogName, req.method, e.message.red);
            if (e.stack) {
          ...
          var write = function (response) {
            ...
            log.debug('W↩'.green, msgLogName, req.method, ('(' + timeTaken + 'ms)').grey);
            return stream.write.call(stream, JSON.stringify(obj));
          };
          var end = function () {
            var obj, timeTaken;
            obj = {
            ...
            timeTaken = Date.now() - req.receivedAt;
            log.debug('E↩'.green, msgLogName, req.method, ('(' + timeTaken + 'ms)').grey);
            return stream.write.call(stream, JSON.stringify(obj));
          };
          ...
          try {
            return request(req, function(err, response) {
              ...
              timeTaken = Date.now() - req.receivedAt;
              log.debug('↩'.green, msgLogName, req.method, ('(' + timeTaken + 'ms)').grey);
              return send(JSON.stringify(obj));
            }, new RpcStream(stream, write, end, handleError));
          } catch (e) {
