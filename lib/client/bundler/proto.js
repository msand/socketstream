    ...
    useLatestsPackedId : function() {
      if (options.packedAssets) {
        try {
          ...
          var files = fs.readdirSync(path.join(ss.root, options.dirs.assets, clientName));
          var stats = files.map(function(file) {
            var stat = fs.statSync(path.join(ss.root, options.dirs.assets, clientName, file));
